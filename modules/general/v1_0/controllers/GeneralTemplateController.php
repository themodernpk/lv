...
class GeneralTemplateController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->data->prefix = "general-template";
        ...
        $this->data->table = 'gn_templates';
        $this->data->input = (object)Input::all();
        $this->data->model = "GeneralTemplate";
        $this->data->view = "general::template.";
    }
    ...
    function index()
    {
        ...
        $list = GeneralTemplate::categoryAutoComplete();
        ...
        return View::make($this->data->view .'general-template-list')->with('title', 'General Template List')
            ->with('data', $this->data);
    }
    ...
    function create()
    {
        ...
        foreach($this->data->input as $key => $val)
        {
            ...
            if($key == 'message')
            {
                if (strpos(strtolower($this->data->input->category),'sms') !== false) {
                    $val = strip_tags(trim($val));
                    $val = CoreExtractor::cleanText($val);
                }
            }
    ...
    function read($id)
    {
        ...
        if($item)
        {
            ...
            if (strpos(strtolower($item->name),'sms') !== false )
            {
                $item->message = strip_tags(trim($item->message));
