...
class CoreSmtp
{
    ...
    public static function send_email($from, $to, $subject, $message, $cc=NULL, $bcc=NULL)
    {
        ...
        $hostname = Setting::value('core-smtp-hostname');
        if(!$hostname)
        {
            $response['status'] = 'failed';
        ...
        $username = Setting::value('core-smtp-username');
        if(!$username)
        {
            $response['status'] = 'failed';
        ...
        $password = Setting::value('core-smtp-password');
        if(!$password)
        {
            $response['status'] = 'failed';
        ...
        $port = Setting::value('core-smtp-port');
        if(!$port)
        {
            $response['status'] = 'failed';
        ...
        try{
            ...
            $encryption = Setting::value('core-smtp-encryption');
